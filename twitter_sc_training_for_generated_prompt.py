...
from src.model.modules_for_prompt import image_model_name
...
def main(rank, args):
    ...
    if args.model_config is not None:
        ...
        # bart_config = MultiModalBartConfig.from_json_file(args.model_config)
    else:
    ...
    if args.checkpoint:
        ...
        seq2seq_model = MultiModalBartModel_AESC(config=bart_config, args=args,
                                                 bart_model=args.bart_model, tokenizer=tokenizer,
                                                 label_ids=label_ids)
        ...
        seq2seq_model = MultiModalBartModel_AESC(bart_config, args,
                                                 ...
                                                 label_ids)
        # 需要一个适配器转换一下这个部分的模型类型
        # 定义的模型MultiModalBartModel_AESC 包含解码器和编码器部分 然后转成生成序列的模型 需要多定义一个类型
        model = SequenceGeneratorModel(seq2seq_model, seq2seq_model,
                                       bos_token_id=bos_token_id,
                                       eos_token_id=eos_token_id,
                                       max_length=args.max_len,
