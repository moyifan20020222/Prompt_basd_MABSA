...
# ------------
    ...
    def encode_condition(self, task, img_num=None, caption=None, sentence=None, has_prompt=False, aspects_num=None,  text_only=False):
        ...
        分别为每一段构建token，image_text部分设计了关于图像表示的部分 通过begin img 和endimg作为头尾
        中间的部分是后续图像表示放入的部分 。 Caption部分 论文说了需要使用， 但是这里都设置了False又不使用了
        sentence是 数据集中的文本部分 在每一个句子开头增加 bos token 末尾增加eos token
        has token 是放在图像和 文本之间的一组提示内容。依据不同任务有不同内容
        ...
        if has_prompt:
            ...
            for index, value in enumerate(aspects_num):
                ...
                # 开头是 there is / are
                for i in range(value):  # 对于不同情况
                    if task == 'AESC':  # 如果是联合任务，首先需要两个生成索引的Prompt
                        ...
                        # 一组内容是 a b of sentiment 为需要生成的三元组信息，索引 + 情绪类型
                        if i < (value-1):  # 每组之间需要分割
                            aspect_prompt_bpes.append([self.sep_token_id])
                    elif task == 'twitter_sc':  # 如果是单纯的识别情绪，因为索引已知，只需要保留情绪，
                        aspect_prompt_bpes.append([self.senti_prompt_token_id])
                        ...
                        if i <(value-1):
                            aspect_prompt_bpes.append([self.sep_token_id])
                    elif task == 'twitter_ae':  # 如果单纯识别索引，同理。
                        aspect_prompt_bpes.append([self.aspect_prompt_token_id]*2)
                ...
                # 后续在添加一个 结束的token 和 in 指示需要从后面文本获取内容
        if image_text is not None:  # 对于构建的img Prompt部分，对他进行token to id的过程。（其他已经先处理过了）
        ...
        # build mlm mask  这个部分放出了文本内容部分，我们添加的代码需要这个部分
        if sentence is not None:
    ...
    def encode_twitter_sc(self, label, aesc_spans, aesc_max_len):
        ...
        # 只将bos_token 和情绪Prompt部分的辨识Token做一个embedding
        senti_prompt_input_text = self.encode(senti_prompt_input_text,
                                     add_special_tokens=False,
                                     return_tensors='pt',
                                     padding=True)
