...
from transformers import GPT2Tokenizer
    # AutoFeatureExtractor, CLIPFeatureExtractor
import logging
...
class Twitter_Dataset(data.Dataset):
    def __init__(self, infos, split, image_model_name):
        ...
        print("目前采用的处理视频模态的模型是：", image_model_name)
    ...
    def __getitem__(self, index):
        ...
        #  修改部分：data/k-shot-has-aspect-num-ACL-2023/twitter_2015 +  /images/test/1739565.jpg
        # print("图片的文件夹路径", self.img_region_dir)
        # print("数据集信息", data['image_path'].lstrip('/').replace('/', os.sep))
        image_path = os.path.join(self.img_region_dir, data['image_path'].lstrip('/').replace('/', os.sep))
