...
class SequenceGeneratorModel(nn.Module):
    ...
    def __init__(self,
        ...
        增加部分： 当前代码无法处理自定义的BART类作为一个seq2seq的model类，进而会导致调用错误，故多添加一个参数用于接受这个自定义的BART类
        """
        super().__init__()
    ...
    def forward(self,
        ...
        :return:
        """
        # 最终还是回到每一个多模态任务下的 比如MultiModalBartModel_AESC 的forward函数下。
    ...
    def predict(self,
        ...
        # 这里的封装会有问题， 接受的MultiModalBartModel_AESC 并不是一个seq2seqmodel的类型 需要做一个修改
