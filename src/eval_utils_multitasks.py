...
def eval_with_pseudo_ctta(args, model, loader, metric, device, num_ctta_steps=1, ctta_lr=1e-5): # 添加 tokenizer, mlm_loss_module, num_ctta_steps, ctta_lr
    num_correct = 0
    ...
    for ctta_step in range(num_ctta_steps):  # Pseudo-CTTA 迭代步骤
        for i, batch in enumerate(loader):  # 遍历 *测试集* loader
            ...
            if args.task == 'twitter_ae':
                ...
                aesc_infos = {key: value for key, value in batch['AESC'].items()}
            predict, predict_aspects_num, pseudo_loss = model.module.predict(  # 评估阶段不需要 pseudo_loss
                input_ids=batch['input_ids'].to(device),
                image_features=list(
                    map(lambda x: x.to(device), batch['image_features'])),
                attention_mask=batch['attention_mask'].to(device),
